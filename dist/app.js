...
import { getDashboard, generateApiKeyRoute, getCompanies, login, register, generateApiKey, toggleCompanyStatus, getApiKey } from './controllers/auth.controller.js';
import { authenticate } from './middleware/auth.middleware.js';
import { getDashboardStats, getSettings, updatePassword, updateProfile } from './controllers/dashboard.controller.js';
...
app.get('/api/auth/github/callback', passport.authenticate('github', { session: false }), (req, res) => {
    const user = req.user;
    const token = jwt.sign({ companyId: user.id, email: user.email }, process.env.JWT_SECRET, { expiresIn: '7d' });
