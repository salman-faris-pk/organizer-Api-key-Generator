...
const Sidebar: React.FC = () => {
  ...
  return (
    <>
      ...
      <button
        onClick={() => setIsCollapsed(!isCollapsed)}
        className="lg:hidden fixed top-4 left-4 z-50 p-2 rounded-md bg-gray-200 text-gray-800"
      >
        {isCollapsed ? <FiX size={24} /> : <FiMenu size={24} />}
      </button>
      ...
      <aside
        className={`
          fixed top-0 left-0 z-40 h-screen bg-white text-gray-800
          transition-transform duration-300 ease-in-out
          ${isCollapsed ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}
      ...
      >
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-gray-200">
          <div className="relative">
            ...
            {/* Dropdown Menu */}
            {isProfileOpen && (
              <div className="absolute bottom-full left-0 right-0 mb-2 bg-white rounded-lg shadow-md overflow-hidden border border-gray-200">
                <button
                  onClick={() => {
                    logout();
                    setIsProfileOpen(false);
